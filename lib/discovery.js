...
LifxDiscovery.prototype.discover = function() {
	...
	// setTimeout(function() {
		// console.log("sending Hall to on");
		// var tags = new Buffer([01, 00, 00, 00, 00, 00, 00, 00]);
		// message = packet.setPowerState({onoff:1});
		// lx.sendToOne(message, tags);
	// }, 2000);
	// setTimeout(function() {
		// console.log("sending Hall to off");
		// var tags = new Buffer([01, 00, 00, 00, 00, 00, 00, 00]);
		// message = packet.setPowerState({onoff:0});
		// lx.sendToOne(message, tags);
	// }, 4000);
};
...
LifxBus.prototype._handleStatus = function(id, status) {
	if (!this.bulbs[id] || !deepEqual(this.bulbs[id], status)) {		// emit event only if new status or status changed
		if (TRACE && DETAIL) {
			console.log("LifxBus: status changed for " + id + " : " + JSON.stringify(status));
		}
