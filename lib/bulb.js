...
var LifxBulb = module.exports = function LifxBulb(def) {
	...
	this.lxBus.on(this.getContentValue("id"), function(status) {
		...
		if (status.name != self.getContentValue("name")) {
...
//		if (self.state.saturation == 0) {
//			console.log("no saturation, use kelvin");
//		}
...
LifxBulb.prototype._handleLifxState = function(state) {
	if (TRACE && DETAIL) {
		...
		//console.log("LifxBulb: existing state: " + JSON.stringify(this.state));
	}
	...
	else if (this.state.kelvin != state.kelvin) {
		hasStateChanged = true;
	}
	...
	if (hasStateChanged) {
		...
		this.state.kelvin = state.kelvin;

		// send brightness on output facet
